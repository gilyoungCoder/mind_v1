...
class enc_mtan_rnn(nn.Module):
    def __init__(self, input_dim, query, latent_dim=2, nhidden=16, 
        ...
        self.dec = dec_mtan_rnn(self.dim, self.query, latent_dim, 50, embed_time=128, learn_emb=True, num_heads=1)
    ...
    def forward(self, x, time_steps):
        ...
        x = torch.cat((pred_x, fmask), 2)
        fmask = torch.cat((fmask, fmask), 2)
        out = self.att(query, key, x, fmask)
        out, _ = self.gru_rnn(out)
        out = self.hiddens_to_z0(out)
